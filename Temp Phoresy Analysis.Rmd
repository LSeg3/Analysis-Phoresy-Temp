...
check3<-datarem%>%group_by(id2, treat_ass2) %>% summarize(counting=n()) 
...
check4<-check3%>% group_by(treat_ass2) %>%summarize(suma=sum(counting))
check4
...
resumentemp<-data%>%filter(response!="Dead", response!="Escaped") %>% 
...
# With the re-assigned treatment based on the tmean
resumentemp<-data%>%filter(response!="Dead", response!="Escaped") %>% 
  group_by(treat_ass4, response) %>%
  summarize(n=n())
resumentemp

ggplot(data=resumentemp, aes(x=treat_ass4, y=n, fill=response))+ geom_bar(stat="identity")

ggplot(data=resumentemp, aes(x=treat_ass4, y=n))+ geom_bar(stat="identity")+facet_wrap(~response)

```

```{r}
# visualizing grabbing data
ggplot(aes(y=grab, x=tmean), data=data)+geom_point(alpha=0.5, size=2)+facet_wrap(~stage)+geom_smooth()


#Visualizing releasing data
ggplot(aes(y=rel, x=tmean), data=data2)+geom_point(alpha=0.5, size=2)+facet_wrap(~stage)+geom_smooth()

# visualizing grabbing data
ggplot(aes(y=grab, x=treat_ass4, color=stage), data=data)+geom_boxplot()
...
# Including stage
m1c<-glmer(rel~treat_ass2*stage+(1|id2), data=data, family=binomial)
summary(m1c)
